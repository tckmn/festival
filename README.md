...
23. `minesweeper`: given a Minesweeper grid, fill in its numbers.

24. `markdown`: given a Markdown string, render it as HTML.

25. `boxes`: given a list of integers, output a series of boxes of the given
    sizes.

26. `differences`: given a list of integers, repeatedly take the differences
    between consecutive pairs.

27. `censor`: given a string and a list of words, censor each of the words in
    the string.

28. `chrtrans`: given an integer and a note, chromatically transpose it by
    the given number of steps.

29. `recaman`: given an integer n, output the first n terms of Recamán's
    Sequence.

30. `cover`: given a set of points, output the area of the smallest rectangle
    that covers them.

31. `sentence`: given a sentence, capitalize the appropriate letters.

32. `binom`: given two coefficients and an exponent, output the binomial
    expansion.
